...
fn main() {
    ...
    let mut w = Vec3{x:0.0, y:0.0, z:0.0};

    // Normal vectors benchmark
    microbench::bench(&options, "add Vec3 normal", || add_vec3(&u, &v, &mut w));
    ...
    let mut c = Vec3Optimized{register: unsafe {_mm_set_ps(0.0, 0.0, 0.0, 0.0)}};
